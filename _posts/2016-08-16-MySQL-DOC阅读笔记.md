...
6.  多列索引的实现: A multiple-column index can be considered a sorted array, the rows of which contain values that are created by concatenating the values of the indexed columns. 
7. 多列索引详情见[Multiple Index](http://dev.mysql.com/doc/refman/5.7/en/multiple-column-indexes.html)
8. Mysql对Where的优化详情见[Where Optimization](http://dev.mysql.com/doc/refman/5.7/en/where-optimizations.html)
9. InnoDB的聚簇索引含有列数据，优先建立组合索引而不是每一列的单独索引。InnoDB的二级索引的叶子节点中存储的是PK，而不是行号。MyISAM的索引是非聚簇索引，叶子节点中是行指针。[See more](http://www.cnblogs.com/hustcat/archive/2009/10/28/1591648.html)
